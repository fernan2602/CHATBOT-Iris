...
class PreguntasPersona: 
        def __init__(self):
                
            self.base_conocimiento = {
        "nombre":["llama", "nombre", "apodo"],
        "correoElectronico":["correo","electronico","gmail"],
        "nroCelular":[ "telefono", "numero","contacto"],
        "Docente":["alumno", "docente","maestro","estudiante"],
        "dni":["dni","documento","identificacion"],
        "apellido":["apellido","apellida","last name"]
        }
        ...
        #Creacion de un bucle que recorre todas las preguntas y respuestas predeterminadas
            for pregunta_usuario in pregunta_completa:
               if (pregunta_completa == "profesor" or "docente" or "maestro"):
                    params = "si"
        ...
        #Ponemos un minimo del 80% para comprobar similitud
            if mejor_coincidencia and mejor_similitud >= 0.8: 
                ...
                cPersona = Consultas()
                print(mejor_coincidencia+" "+params)
      

                dato = cPersona.search_by_all(mejor_coincidencia,tabla,params)
